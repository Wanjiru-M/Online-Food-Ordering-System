...
$sql $conn->query("UPDATE orders SET Status CANCELLED WHERE orders. ID $ID"); $rs $sql->execute();
}else{
$sql $conn->query("UPDATE orders SET 'Status 'SUCCESS WHERE orders. ID $ID"); $rs - $sql->execute();
}
if($rs){
